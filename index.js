...
const msgTemplates = require('./messages');
...
async function runNurtureScan(onlyUserId) {
  ...
  try {
    ...
    for (const u of users) {
      const due = decideNurture(u, { bypassCutoff: !!onlyUserId });
      if (!due) continue;
...
app.get('/run-nurture', async (req, res) => {
  // 接受 ADMIN_KEY 或（選用、較單純）NURTURE_RUN_KEY；兩邊都 trim，避免 Render 貼上多空白/換行
